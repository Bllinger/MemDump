//
// Created by Blinger on 2018/8/23.
//
/*
 ============================================================================
 Name        : libinject.c
...
//  target_pid = find_pid_of("/data/injected");
    if(argc < 3){
        puts("Input Process ID!");
        return -1;
    }
    target_pid = atoi(argv[1]);
    libname = argv[2];
    //sprintf(libname,"/data/data/%s",argv[2]);
    //function_name = argv[3];
    //printf("libname = %s\nfuncname = %s\n", libname,function_name);
    inject_remote_process( target_pid, "/data/data/libdump.so", "startDump", libname, strlen(libname));
}
